...
server.tool(
  "nft-top-holders",
  "Get the list of holders for a specific NFT on Monad Testnet using BlockVision API",
  {
  ...
  async ({ contractAddress, tokenId, standard, limit }) => {
    try {
      ...
      while (hasMore && allHolders.length < maxHolders) {
        try {
          ...
          const response = await axios.get(
            `${BLOCKVISION_BASE_URL}/monad/collection/holders?contractAddress=${contractAddress}&pageIndex=${pageIndex}&pageSize=${pageSize}`,
            {
              headers: {
          ...
          console.error(
            `Retrieved ${holders.length} holders on page ${pageIndex} (Total: ${totalRetrieved})`
          );

          // Check if we've reached the end or max holders
          if (
            !response.data.result.nextPageIndex ||
            holders.length < pageSize ||
            allHolders.length >= maxHolders
          ) {
            hasMore = false;
            console.error(
              `Finished retrieving top holders. Total: ${Math.min(
                allHolders.length,
                maxHolders
              )}`
            );
          ...
          if (axios.isAxiosError(error)) {
            if (error.response?.status === 404) {
              throw new Error(
                `Contract address ${contractAddress} not found or not an NFT contract`
              );
            }
      ...
      const holdersList = topHolders
        .map(
          (holder, index) =>
            `${index + 1}. ${holder.ownerAddress} (${holder.amount} tokens, ${
              holder.percentage
            }%)`
        )
        .join("\n");
        ...
        content: [
          {
            ...
            text: `Top ${
              topHolders.length
            } NFT holders for contract ${contractAddress}:\n\n${
              topHolders.length > 0
