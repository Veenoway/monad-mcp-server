...
server.tool(
  "portfolio",
  "Analyze a complete portfolio of an address on Monad Testnet",
  {
  ...
  async ({
    ...
    try {
      ...
      const result: {
        ...
        transactions: any[];
        holdingHistory: { balance: string; timestamp: number }[];
        performanceMetrics: {
          totalPnL: string;
      ...
      try {
        const response = await axios.get(
          `${BLOCKVISION_BASE_URL}/account/tokens?address=${address}`,
          {
            headers: {
      ...
      try {
        const response = await axios.get(
          `${BLOCKVISION_BASE_URL}/account/nfts`,
          {
            headers: {
              "x-api-key": BLOCKVISION_API_KEY,
            },
        ...
        if (response.data.code === 0) {
          ...
          console.error(
            `Found ${result.nfts.erc721.length} ERC721 NFTs and ${result.nfts.erc1155.length} ERC1155 NFTs`
          );
        }
      } catch (error) {
      ...
      if (includeTransactionHistory) {
        try {
          const response = await axios.get(
            `${BLOCKVISION_BASE_URL}/account/transactions`,
            {
              headers: {
                "x-api-key": BLOCKVISION_API_KEY,
              },
          ...
          if (response.data.code === 0) {
            result.transactions = response.data.result.data.map((tx: any) => ({
              hash: tx.hash,
            ...
            const holdingHistoryMap: {
              [key: number]: { balance: string; timestamp: number };
            } = {};
            ...
            result.holdingHistory = Object.values(holdingHistoryMap).sort(
            ...
            result.performanceMetrics = {
              ...
              largestTransaction: ethers.formatEther(
            ...
            console.error(
              `Found ${result.transactions.length} recent transactions and ${result.holdingHistory.length} balance changes`
            );
