...
server.tool("swap", "Allow users to swap tokens on DEXes available on Monad Testnet", {
    privateKey: z
        .string()
    ...
    amountIn: z
        .string()
        .describe("Input token amount (in whole units, will be converted according to token decimals)"),
    ...
    try {
        ...
        if (!useNativeMON && !tokenInAddress) {
            throw new Error("Input token address is required when useNativeMON is false");
        }
        ...
        if (!ethers.isAddress(routerAddress)) {
            console.error(`ERROR: Invalid ${routerType} router address: ${routerAddress}`);
            throw new Error(`Invalid or unconfigured ${routerType} router address. Please check DEX configuration.`);
        }
        if (!ethers.isAddress(WMON_ADDRESS)) {
            console.error(`ERROR: Invalid WMON address: ${WMON_ADDRESS}`);
            throw new Error(`Invalid or unconfigured WMON address: ${WMON_ADDRESS}. Please check configuration.`);
        }
        ...
        if (routerCode === "0x" || routerCode === "") {
            console.error(`ERROR: No code at ${routerType} router address: ${routerAddress}`);
            throw new Error(`No contract exists at ${routerType} router address (${routerAddress}). Verify that the DEX is properly deployed on Monad testnet.`);
        }
        ...
        async function checkPoolLiquidity(tokenA, tokenB) {
            try {
                ...
                console.error(`Liquidity: ${ethers.formatEther(reserveA)} ${tokenASymbol} <-> ${ethers.formatEther(reserveB)} ${tokenBSymbol}`);
                return {
                    exists: true,
                    ...
                    try {
                        ...
                        if (reserve0 > ethers.parseEther("0.01") &&
                            ...
                            try {
                                ...
                                console.error(`Error retrieving symbols for pair ${i}:`, error);
                            }
                        }
                    }
                    catch (error) {
  ...
  Direct pool ${effectiveTokenInAddress === WMON_ADDRESS
                            ? "MON"
                            : directPoolInfo.tokenASymbol} -> ${directPoolInfo.tokenBSymbol}: ${directPoolInfo.exists ? "Exists" : "Does not exist"}
  ...
  Pool ${effectiveTokenInAddress === WMON_ADDRESS
                            ? "MON"
                            : wmonPoolInfoIn.tokenASymbol} -> MON: ${wmonPoolInfoIn.exists ? "Exists" : "Does not exist"}
  ${wmonPoolInfoIn.exists ? `Liquidity: ${wmonPoolInfoIn.liquidity}` : ""}
  
  Pool MON -> ${wmonPoolInfoOut.tokenBSymbol}: ${wmonPoolInfoOut.exists ? "Exists" : "Does not exist"}
  ${wmonPoolInfoOut.exists ? `Liquidity: ${wmonPoolInfoOut.liquidity}` : ""}
  
  Recommended path: ${directPoolInfo.exists
                            ? "Direct"
  ...
  If you want to create liquidity, you will need to add tokens to the pools through the DEX interface.`
        ...
        if (useNativeMON) {
            ...
            console.error(`Using MON -> Token path: ${WMON_ADDRESS} -> ${tokenOutAddress}`);
            if (!wmonPoolInfoOut.exists) {
                throw new Error(`No liquidity pool exists for MON -> ${wmonPoolInfoOut.tokenBSymbol || tokenOutAddress}. Cannot proceed with swap.`);
            }
        }
        else if (tokenOutAddress.toLowerCase() === WMON_ADDRESS.toLowerCase()) {
            path = [tokenInAddress, WMON_ADDRESS];
            console.error(`Using Token -> MON path: ${tokenInAddress} -> ${WMON_ADDRESS}`);
            if (!wmonPoolInfoIn.exists) {
                throw new Error(`No liquidity pool exists for ${wmonPoolInfoIn.tokenASymbol || tokenInAddress} -> MON. Cannot proceed with swap.`);
            }
        }
        else {
            if (directPoolInfo.exists) {
                ...
                useDirectPath = true;
                console.error(`Using direct path: ${tokenInAddress} -> ${tokenOutAddress}`);
            }
            else if (wmonPoolInfoIn.exists && wmonPoolInfoOut.exists) {
                path = [tokenInAddress, WMON_ADDRESS, tokenOutAddress];
                console.error(`Using path via MON: ${tokenInAddress} -> ${WMON_ADDRESS} -> ${tokenOutAddress}`);
            }
            else {
                throw new Error(`No viable swap path found between ${wmonPoolInfoIn.tokenASymbol || tokenInAddress} and ${wmonPoolInfoOut.tokenBSymbol || tokenOutAddress}. Verify that liquidity pools exist on ${routerType}.`);
            }
        }
        ...
        if (useNativeMON) {
            ...
            console.error(`Wallet MON balance: ${ethers.formatEther(userBalance)} MON`);
        }
        else {
            ...
            console.error(`Wallet token balance: ${ethers.formatUnits(userBalance, decimals)} tokens`);
        }
        let formattedAmountIn;
        let tokenDecimals = 18;
        if (useNativeMON) {
            formattedAmountIn = ethers.parseEther(amountIn);
            console.error(`Input amount: ${amountIn} MON (${formattedAmountIn.toString()} wei)`);
            const estimatedGas = ethers.parseEther("0.005");
            if (userBalance < formattedAmountIn + estimatedGas) {
                throw new Error(`Insufficient MON balance. You have ${ethers.formatEther(userBalance)} MON, but trying to swap ${amountIn} MON plus gas fees.`);
            }
        }
        else {
            const tokenIn = new ethers.Contract(tokenInAddress, ERC20_ABI, provider);
            ...
            console.error(`Input amount: ${amountIn} tokens (${formattedAmountIn.toString()} units with ${tokenDecimals} decimals)`);
            if (userBalance < formattedAmountIn) {
                throw new Error(`Insufficient token balance. You have ${ethers.formatUnits(userBalance, tokenDecimals)} tokens, but trying to swap ${amountIn}.`);
            }
        }
        let amountsOut;
        let estimatedAmountOut;
        try {
            console.error(`Requesting getAmountsOut for ${ethers.formatUnits(formattedAmountIn, useNativeMON ? 18 : tokenDecimals)} with path:`, path);
            ...
            if (estimatedAmountOut === BigInt(0)) {
                throw new Error("Estimated output amount is 0, suggesting no liquidity");
            }
            console.error(`Amounts calculated by router:`, amountsOut.map((a) => a.toString()));
        }
        catch (error) {
            console.error(`Error estimating amounts:`, error);
            throw new Error(`Could not estimate output amount. This is usually due to lack of liquidity for this trading pair or an issue with the DEX router.`);
        }
        const slippageFactor = 1000 - 5 * 10;
        const minAmountOut = (estimatedAmountOut * BigInt(slippageFactor)) / BigInt(1000);
        console.error(`Estimated output amount: ${ethers.formatUnits(estimatedAmountOut, 18)}`);
        console.error(`Minimum output amount (with ${slippagePercentage}% slippage): ${ethers.formatUnits(minAmountOut, 18)}`);
        ...
        try {
            if (useNativeMON) {
                ...
                if (allowance < formattedAmountIn) {
                    ...
                    console.error(`Token approved successfully. Hash: ${approveTx.hash}, Block: ${approveReceipt.blockNumber}`);
                }
                else {
                    console.error(`Existing approval sufficient: ${allowance.toString()}`);
                }
            ...
            console.error(`Transaction pending confirmation: ${swapTx.hash ?? swapTx}`);
            // In ethers.js v6, we need to use provider.waitForTransaction instead of wait()
            const txHash = typeof swapTx === "object" && swapTx !== null
                ? "hash" in swapTx
                    ? swapTx.hash
                    : String(swapTx)
                : String(swapTx);
            const receipt = (await provider.waitForTransaction(txHash));
            console.error(`Transaction confirmed! Hash: ${receipt?.hash ?? "unknown"}`);
        }
        catch (error) {
            ...
            throw new Error(`Swap failed: ${errorMessage}. Technical details: ${errorString}`);
        }
