...
server.tool("swap", "Permettre aux utilisateurs d'échanger des tokens sur des DEX disponibles sur Monad Testnet", {
    privateKey: z
        .string()
  ...
  Si vous souhaitez créer de la liquidité, vous devrez ajouter des tokens aux pools via l'interface du DEX.`
        ...
        try {
            ...
            console.error(`Transaction en attente de confirmation: ${swapTx.hash ?? swapTx}`);
            ...
            console.error(`Transaction confirmée! Hash: ${receipt?.hash ?? "inconnu"}`);
        }
        catch (error) {
