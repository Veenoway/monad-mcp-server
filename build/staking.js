...
server.tool("monad-staking", "Staker des tokens MON sur Monad Testnet", {
    privateKey: z
        .string()
    ...
    try {
        ...
        console.error(`Preparing to stake ${amount} MON from ${walletAddress}...`);
        const balance = await provider.getBalance(walletAddress);
        if (balance < amountToStake) {
            throw new Error(`Solde insuffisant: ${ethers.formatEther(balance)} MON disponible, ${amount} MON requis`);
        }
        const aprMonCode = await provider.getCode(APR_MON_ADDRESS);
        if (aprMonCode === "0x" || aprMonCode === "") {
            throw new Error(`APR_MON contract is not deployed at address ${APR_MON_ADDRESS}`);
        }
        ...
        console.error(`Contract MON balance: ${ethers.formatEther(contractBalance)} MON`);
        const aprMonContract = new ethers.Contract(APR_MON_ADDRESS, APR_MON_ABI, wallet);
        const currentAprMonBalance = await aprMonContract.balanceOf(walletAddress);
        console.error(`Current aprMON balance: ${ethers.formatEther(currentAprMonBalance)} aprMON`);
        const sharesToReceive = await aprMonContract.convertToShares(amountToStake);
        console.error(`You will receive ${ethers.formatEther(sharesToReceive)} aprMON`);
        try {
        ...
        let gasEstimate;
        try {
            gasEstimate = await aprMonContract.deposit.estimateGas(amountToStake, walletAddress, {
                value: amountToStake,
            });
        ...
        const depositTx = await aprMonContract.deposit(amountToStake, walletAddress, {
            value: amountToStake,
...
server.tool("monad-unstaking", "Unstake MON tokens from Monad Testnet", {
    privateKey: z
        .string()
    ...
    try {
        ...
        console.error(`Preparing to unstake ${amount} aprMON from ${walletAddress}...`);
        const aprMonContract = new ethers.Contract(APR_MON_ADDRESS, APR_MON_ABI, wallet);
        const aprMonBalance = await aprMonContract.balanceOf(walletAddress);
        if (aprMonBalance < amountToUnstake) {
            throw new Error(`Insufficient balance: ${ethers.formatEther(aprMonBalance)} aprMON available, ${amount} aprMON required`);
        }
        const requestTx = await aprMonContract.requestRedeem(amountToUnstake, walletAddress, walletAddress);
        ...
        try {
            requestId = requestReceipt.logs
                ...
                .map((log) => {
                const decoded = aprMonContract.interface.decodeEventLog("RequestRedeem", log.data, log.topics);
        ...
        if (!requestId) {
            console.error("Request ID could not be retrieved, but the request was sent successfully");
            return {
                content: [
                    {
                        type: "text",
        ...
        await new Promise((resolve) => setTimeout(resolve, Number(waitTime) * 12000)); // ~12s per block
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `✅ Unstake request for ${amount} aprMON successfully sent from ${walletAddress}\nTransaction: ${redeemTx.hash}\nNew aprMON balance: ${ethers.formatEther(newAprMonBalance)} aprMON\n\n⚠️ The unstaked MON will appear in your wallet in about 10 minutes.`,
                },
            ],
        };
    }
