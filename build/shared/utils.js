 ...
 */
export function detectParamType(param) {
    if (!param)
        return "unknown";
 ...
 */
export function convertConstructorArg(value, paramType) {
 ...
 */
export function detectProxyPattern(bytecode) {
    const patterns = [];
    // Recherche du modèle EIP-1967
    if (bytecode.includes("5c60da1b") || // implementation slot
    ...
    if (bytecode.includes("5c19a95c") || // admin slot
        bytecode.includes("delegatecall")) {
        patterns.push("Transparent Proxy");
    }
    // Recherche du modèle UUPS Proxy
    if (bytecode.includes("52d1902d") || // UUPSUpgradeable interface
 ...
 */
export function detectContractStandards(bytecode) {
    const standards = [];
    // ERC-20
    if (bytecode.includes("a9059cbb") || // transfer
    ...
    // ERC-721
    if (bytecode.includes("6352211e") || // ownerOf
    ...
    // ERC-1155
    if (bytecode.includes("f242432a") || // safeTransferFrom
    ...
    // ERC-777
    if (bytecode.includes("959b8c3f") || // authorizeOperator
