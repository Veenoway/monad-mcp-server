...
server.tool("defi-challenges", "Participate in DeFi challenges on Monad and compare your performance with other users", {
    ...
    try {
        console.error(`Initializing DeFi challenge for ${publicUsername || "anonymous user"}`);
        // Initialize and validate parameters
        if (!ethers.isAddress(initialInvestment) &&
            isNaN(Number(initialInvestment))) {
            throw new Error(`Invalid amount for initial investment: ${initialInvestment}`);
        }
        ...
        if (balance < investmentAmount) {
            throw new Error(`Insufficient balance. You have ${formattedBalance} MON, but you're trying to invest ${initialInvestment} MON.`);
        }
        // Simulate challenge registration
        console.error(`Registering for ${challengeType} challenge with ${duration} duration`);
...
function generatePerformanceASCIIGraph(performanceData) {
    const height = 10;
    const width = 50;
    // Extract values
    ...
    const baselineY = height - 1 - Math.floor(((100 - min) / range) * (height - 1));
    ...
    // Add labels
    const result = `${max.toFixed(1)}% ┌${"─".repeat(width)}┐\n${graphStr}\n${min.toFixed(1)}% └${"─".repeat(width)}┘`;
    return result;
}
